...
const must=async(path,{status=200,contains=[]}={})=>{
  const r=await fetch(origin+path,{redirect:'error'});const text=await r.text();
...
const session=await fetch(origin+'/api/session',{redirect:'error'});const sj=await session.json();
if(session.status!==200||sj.configured!==true||sj.authenticated!==false)throw Error(`session gate mismatch: ${session.status} ${JSON.stringify(sj)}`);
console.log('PASS GET /api/session configured=true authenticated=false');
const rpc=await fetch(origin+'/api/rpc',{method:'POST',redirect:'error',headers:{Origin:origin,'Content-Type':'application/json'},body:JSON.stringify({action:'admin.ops.events',payload:{limit:1}})});let body={};try{body=await rpc.json();}catch{}
if(rpc.status!==401||body.error!=='UNAUTHENTICATED')throw Error(`rpc fail-closed mismatch: ${rpc.status} ${JSON.stringify(body)}`);

...
const ADMIN_ACTIONS=new Set([
  'admin.overview','admin.audit','admin.access.list','admin.access.save','admin.farm.delete','admin.account.delete','admin.ops.events','admin.ops.review'
]);
const DECK_ACTIONS=new Set(['deckAssets','deckBootstrap','saveDeck']);
...
async function dispatchDeckAction(env,principal,action,payload,fetchImpl){
  switch(action){case 'deckAssets':return deckAssets(env,principal,fetchImpl);case 'deckBootstrap':return deckBootstrap(env,principal,fetchImpl);case 'saveDeck':if(!DECK_WRITE_GATE_OPEN)throw Error('DECK_WRITE_GATE_CLOSED');return saveDeck(env,principal,payload,fetchImpl);default:throw Error('STAGING_DECK_ACTION_NOT_IMPLEMENTED');}
}

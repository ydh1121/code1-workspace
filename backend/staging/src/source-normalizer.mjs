...
export function normalizeSource(source){
  ...
  const auditLog=(source.accessLog||[]).filter(r=>r.action).map(r=>({at:timestamp(r.at),actor_id:str(r.actor_id)||null,action:str(r.action),target_type:auditTarget(str(r.action)),target_id:str(r.target_id)||null,detail:str(r.detail)||null,request_id:null,metadata:{source:'20_WEB_ACCESS_LOG'}}));
  ...
  const mediaEvents=(source.mediaHistory||[]).filter(r=>r.upload_id&&!deckIds.has(str(r.upload_id))).map(r=>({at:timestamp(r.at),actor_id:str(r.actor_id)||null,media_id:str(r.upload_id),event_type:`LEGACY_${str(r.status)||'EVENT'}`,from_status:null,to_status:str(r.status)||null,object_key:null,detail:str(r.detail)||null,request_id:null,metadata:{source:'24_WEB_미디어정리_이력',source_drive_file_id:str(r.drive_file_id)||null,media_type:str(r.media_type)||null,category:str(r.category)||null,shot_code:str(r.shot_code)||null,shot_label:str(r.shot_label)||null,original_file_name:str(r.original_file_name)||null,stored_file_name:str(r.stored_file_name)||null,folder_path:str(r.folder_path)||null,legacy_version:Number(r.version||0)}}));

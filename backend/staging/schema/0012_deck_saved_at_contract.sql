...
-- Preserve legacy Deck invariant: payload.updated_at == committed revision saved_at.
-- Applied after 0011_deck_staging_runtime.sql. Never apply to Production.
...
begin
  ...
  if coalesce(v_payload->>'deck_id','') <> p_deck_id
     ...
     or coalesce(v_payload->>'saved_by','') <> v_snapshot

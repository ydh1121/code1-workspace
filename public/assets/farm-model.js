(function(root){
  ...
  function effectiveCatalog(catalog,form){const farmId=form?.farmId||'';return catalog.filter(q=>!['HIDE','NOT_APPLICABLE','PERMANENT_EXCLUDE'].includes(policyMode(q.item_key,farmId)));}
  function progress(catalog,form){const active=effectiveCatalog(catalog,form),done=active.filter(q=>received(q,form)).length;return {done,total:active.length,left:active.length-done,percent:active.length?Math.round(done/active.length*100):0};}
  function questions(catalog,form,{query='',filter='all',category=null}={}){
    const needle=query.trim().toLocaleLowerCase();
    return effectiveCatalog(catalog,form).filter(q=>{
  ...
  function missing(catalog,form){return effectiveCatalog(catalog,form).filter(q=>!received(q,form));}
  root.Code1Farm={groups,hasValue,received,progress,questions,missing,effectiveCatalog,policyMode};
  ...
  function buildUi(){
    ...
    page.innerHTML=`<div class="page-heading"><div><h1>입력 항목 관리</h1><p class="muted">질문 원본은 삭제하지 않고, 전체 또는 농가별로 받을 항목만 정합니다. 이미 받은 답변은 숨겨도 보존됩니다.</p></div></div>
  ...
  function renderPolicies(){
    ...
    let rows=policy.catalog.filter(q=>(!section||q.section_code===section)&&(!query||[q.item_key,q.item_label,q.plain_question,q.why_needed,q.help_text].filter(Boolean).join(' ').toLocaleLowerCase().includes(query)));
    ...
    bySection.forEach((items,code)=>{
      const wrap=el('section','question-policy-section'),heading=el('h2',null,`${code} · ${items[0].section_name||'분류명 미등록'} · ${items.length}개`);wrap.append(heading);
      items.forEach(q=>wrap.append(policyRow(q,s)));root.append(wrap);
    });updateDirtyStatus();
  }
  function policyRow(q,s){
    const draft=currentDraft(q),effective=effectiveWithDraft(q,draft),visible=!['HIDE','NOT_APPLICABLE','PERMANENT_EXCLUDE'].includes(effective),row=el('article','question-policy-row'+(visible?'':' is-hidden'));
    ...
    const modes=s.scope==='GLOBAL'?[['SHOW','입력받기'],['HIDE','전역 숨김'],['PERMANENT_EXCLUDE','영구 제외 · 원본 보존']]:[['INHERIT','전체 기본값 따름'],['SHOW','이 농가에서 입력받기'],['HIDE','이 농가에서 숨김'],['NOT_APPLICABLE','이 농가에는 해당 없음']];modes.forEach(([v,t])=>mode.append(option(v,t)));mode.value=draft.mode;modeLabel.append(mode);
    const reasonLabel=el('label',null,'받지 않는 이유'),reason=document.createElement('select');reason.append(option('','사유 선택'));policy.reasons.forEach(r=>reason.append(option(r.code,r.label)));reason.value=draft.reasonCode;reasonLabel.append(reason);
    const noteLabel=el('label',null,'운영 메모'),note=document.createElement('input');note.type='text';note.maxLength=500;note.value=draft.reasonNote;note.placeholder='왜 받지 않는지 판단 근거';noteLabel.append(note);
    const syncDisabled=()=>{const needs=['HIDE','NOT_APPLICABLE','PERMANENT_EXCLUDE'].includes(mode.value);reason.disabled=!needs;note.disabled=!needs;check.checked=!['HIDE','NOT_APPLICABLE','PERMANENT_EXCLUDE'].includes(effectiveWithDraft(q,{...draft,mode:mode.value}));row.classList.toggle('is-hidden',!check.checked);};
    const changed=()=>{draft.mode=mode.value;draft.reasonCode=reason.value;draft.reasonNote=note.value;if(!['HIDE','NOT_APPLICABLE','PERMANENT_EXCLUDE'].includes(draft.mode)){draft.reasonCode='';draft.reasonNote='';reason.value='';note.value='';}markDirty(draft);syncDisabled();};
    mode.addEventListener('change',changed);reason.addEventListener('change',changed);note.addEventListener('input',changed);check.addEventListener('change',()=>{mode.value=check.checked?(s.scope==='GLOBAL'?'SHOW':'SHOW'):(s.scope==='GLOBAL'?'HIDE':'NOT_APPLICABLE');changed();});syncDisabled();row.append(checkLabel,copy,modeLabel,reasonLabel,noteLabel);return row;
  }
  async function savePolicies(){
    ...
    for(const c of changes){if(['HIDE','NOT_APPLICABLE','PERMANENT_EXCLUDE'].includes(c.mode)&&(!c.reasonCode||!c.reasonNote.trim())){out.textContent=`${c.itemKey}: 받지 않는 이유와 운영 메모를 입력해 주세요.`;return;}}
